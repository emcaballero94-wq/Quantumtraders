...
export function DailyBiasBoard({ assets }: DailyBiasBoardProps) {
  ...
  return (
    <div className="rounded-xl border border-bg-border bg-bg-card glass-card p-4 space-y-3">
      ...
      <div className="space-y-2">
        {topAssets.map((asset, i) => {
          ...
          return (
            <div
              key={asset.symbol}
              className="flex items-center justify-between px-3 py-2 rounded-lg border border-bg-border bg-bg-elevated/30 hover:border-oracle/25 hover:bg-bg-elevated/50 transition-all duration-200 animate-slide-up"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 w-24">
                  <div className="flex-1 h-1 rounded-full bg-bg-deep overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${setup.score >= 70 ? 'bg-atlas' : setup.score >= 50 ? 'bg-oracle' : 'bg-bear'}`}
                      style={{ width: `${setup.score}%` }}
                    />
                  </div>
                  <span className={`text-xs font-mono font-bold tabular-nums w-6 text-right ${setup.score >= 70 ? 'text-atlas' : setup.score >= 50 ? 'text-oracle' : 'text-bear'}`}>
                    {setup.score}
                  </span>
                </div>
              </div>
